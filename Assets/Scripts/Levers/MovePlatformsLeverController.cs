...
public class MovePlatformsLeverController : MonoBehaviour
{
    ...
    void Update()
    {
        if(this.gameObject.GetComponent<LeverController>().isActive && !arePlatformsMoving){
            arePlatformsMoving = true;
            changePlatformsStatus();
        }
        else if(!this.gameObject.GetComponent<LeverController>().isActive && arePlatformsMoving){
